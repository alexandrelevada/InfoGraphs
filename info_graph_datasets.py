...
to their contribution to intra and inter-community structure. By extracting the minimum and maximum information
spanning trees, we isolate structurally relevant edges that respectively reinforce community cohesion and
inter-community bridging. The union of these trees yields the Information Graph (IG), which offers a filtered 
...
def plot_network(G, target, layout, pos=0, leg=False):
    ...
    nx.draw_networkx(G, pos, node_size=node_sizes, node_color=color_map, with_labels=False, width=0.25, alpha=0.35)
...
# Plot network
plt.figure(1)
nx.draw_networkx(G, pos, node_size=25, node_color='black', with_labels=False, width=0.25, alpha=0.35)
plt.show()
